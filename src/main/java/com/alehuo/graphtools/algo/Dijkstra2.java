...
import com.alehuo.graphtools.graph.Edge2;
import com.alehuo.graphtools.graph.Graph2;
...
public class Dijkstra2 {
    ...
    public long shortestDistance(Node n1, Node n2) throws EdgeWeightException {
        ...
        priorityQueue.add(new Edge2(n1, null, dist[n1.getKey()]));

        while (!priorityQueue.isEmpty()) {
            Edge2 n = priorityQueue.poll();
            if (n.getWeight() < 0) {
                throw new EdgeWeightException("Edge weight must not be negative.");
            }
            ArrayList<Edge2> connections = g.getEdgesFrom(n.getStartingNode());
            for (Edge2 connection : connections) {
                ...
                if (!visited[connection.getEndingNode().getKey()]) {
                    ...
                    if (n.getWeight() + connection.getWeight() < dist[connection.getEndingNode().getKey()]) {
                        ...
                        Edge2 tmpNode = new Edge2(connection.getStartingNode(), connection.getEndingNode(), n.getWeight() + connection.getWeight());
